...
def compare_results(df1, df2, tool1, tool2):
    ...
    return merged_df
def calculate_sample_statistics(sample_df, tool1, tool2):
    sample_stats = {}
...
def calculate_statistics(merged_df, tool1, tool2):
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        future_to_sample = {executor.submit(calculate_sample_statistics, merged_df[merged_df['Sample'] == sample], tool1, tool2): sample 
...
def plot_bland_altman(sample_df, col1, col2, metric, tool1, tool2, sample, output_dir):
    ...
    plt.close()
def plot_ma(sample_df, col1, col2, metric, tool1, tool2, sample, output_dir):
    plt.figure(figsize=(10, 8))
    m = np.log2((sample_df[col1] + 1) / (sample_df[col2] + 1))
    a = 0.5 * np.log2((sample_df[col1] + 1) * (sample_df[col2] + 1))
    plt.scatter(a, m, alpha=0.6)
    plt.axhline(y=0, color='r', linestyle='--')
    plt.xlabel(f'A = 1/2*log2({tool1}*{tool2})')
    plt.ylabel(f'M = log2({tool1}/{tool2})')
    plt.title(f'MA Plot of {metric} for {sample}')
    plt.savefig(os.path.join(output_dir, f'{sample}_{metric}_ma_plot.png'))
    plt.close()

def plot_violin(merged_df, metric, tool1, tool2, output_dir):
    plt.figure(figsize=(12, 6))
    data = pd.melt(merged_df, id_vars=['Sample'], value_vars=[f'{metric}_{tool1}', f'{metric}_{tool2}'],
                   var_name='Tool', value_name=metric)
    sns.violinplot(x='Sample', y=metric, hue='Tool', data=data, split=True)
    plt.xticks(rotation=45)
    plt.title(f'Distribution of {metric} values across samples')
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, f'{metric}_violin_plot.png'))
    plt.close()

def plot_cumulative_tpm(merged_df, tool1, tool2, output_dir):
    plt.figure(figsize=(10, 8))
    for tool in [tool1, tool2]:
        tpm_values = merged_df[f'TPM_{tool}'].sort_values(ascending=False)
        cumsum = np.cumsum(tpm_values)
        plt.plot(range(1, len(cumsum) + 1), cumsum / cumsum.max(), label=tool)
    plt.xlabel('Number of transcripts')
    plt.ylabel('Cumulative sum of TPM (normalized)')
    plt.title('Cumulative Distribution of TPM Values')
    plt.legend()
    plt.savefig(os.path.join(output_dir, 'cumulative_tpm_distribution.png'))
    plt.close()

def plot_sample_comparison(sample, sample_df, tool1, tool2, output_dir):
    for metric in ['TPM', 'Count', 'EffLength']:
        col1 = f'{metric}_{tool1}'
        col2 = f'{metric}_{tool2}'
        ...
        plot_ma(sample_df_filtered, col1, col2, metric, tool1, tool2, sample, output_dir)

def plot_comparison(merged_df, tool1, tool2, output_dir):
    os.makedirs(output_dir, exist_ok=True)
    with concurrent.futures.ThreadPoolExecutor() as executor:
        executor.map(partial(plot_sample_comparison, merged_df=merged_df, tool1=tool1, tool2=tool2, output_dir=output_dir), 
                     merged_df['Sample'].unique())
    
    # Add plots for all samples
    for metric in ['TPM', 'Count', 'EffLength']:
        plot_violin(merged_df, metric, tool1, tool2, output_dir)
    plot_cumulative_tpm(merged_df, tool1, tool2, output_dir)
...
def plot_transcript_detection(transcript_counts, tool1, tool2, output_dir):
    ...
    plt.close()
def calculate_summary_statistics(merged_df, tool1, tool2):
    summary_stats = {}
    for metric in ['TPM', 'Count', 'EffLength']:
        col1 = f'{metric}_{tool1}'
        col2 = f'{metric}_{tool2}'
        ...
        pearson_corr, p_value = pearsonr(merged_df[col1], merged_df[col2])
        spearman_corr, _ = spearmanr(merged_df[col1], merged_df[col2])
        
        summary_stats[f'{metric}_Overall_Pearson_Correlation'] = pearson_corr
        summary_stats[f'{metric}_Overall_Pearson_P_Value'] = p_value
        summary_stats[f'{metric}_Overall_Spearman_Correlation'] = spearman_corr
        
        # Calculate mean and median differences
        diff = merged_df[col1] - merged_df[col2]
        summary_stats[f'{metric}_Mean_Difference'] = np.mean(diff)
        summary_stats[f'{metric}_Median_Difference'] = np.median(diff)
        
        # Calculate overall MAE and RMSE
        summary_stats[f'{metric}_Overall_MAE'] = mean_absolute_error(merged_df[col1], merged_df[col2])
        summary_stats[f'{metric}_Overall_RMSE'] = np.sqrt(mean_squared_error(merged_df[col1], merged_df[col2]))
...
def main():
    while True:
        try:
            ...
            summary_stats = calculate_summary_statistics(merged_df, tool1, tool2)
            ...
            with pd.ExcelWriter(excel_file) as writer:
                ...
                pd.DataFrame.from_dict(summary_stats, orient='index').to_excel(writer, sheet_name='Summary_Statistics')
